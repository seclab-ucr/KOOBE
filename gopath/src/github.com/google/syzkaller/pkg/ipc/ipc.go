...
type ProgInfo struct {
	...
	Cap   Capability // stores new capability signal
}
...
func (env *Env) parseOutput(p *prog.Prog) (*ProgInfo, error) {
	...
	info.Cap.Size = 0
	var magic uint32
	if magic, ok = readUint32(&out); ok && magic == 0xdeadbeef {
		if info.Cap.Flags, ok = readUint32(&out); !ok {
			return info, nil
		}
		if info.Cap.Size, ok = readUint32(&out); !ok {
			return info, nil
		}
	}
	return info, nil
