...
func (r *randGen) createResource(s *state, res *ResourceType) (arg Arg, calls []*Call) {
	...
	for i := 0; i < 1e3; i++ {
		...
		calls, ok := r.generateParticularCall(s, meta)
		if !ok {
			continue
		}
...
func (r *randGen) isAllowedType(typ Type, depth int) bool {
	if typ.Dir() == DirOut || typ.Dir() == DirInOut {
		return false
	}
	...
	switch a := typ.(type) {
	case *PtrType:
		return r.isAllowedType(a.Type, depth+1)
	case *StructType:
		for _, subtyp := range a.Fields {
			if !r.isAllowedType(subtyp, depth+1) {
				return false
			}
		}
		return true
...
func (r *randGen) isAllowed(meta *Syscall) bool {
	for _, typ := range meta.Args {
		if !r.isAllowedType(typ, 0) {
			return false
		}
	}
	return true
...
func (r *randGen) generateCall(s *state, p *Prog) ([]*Call, bool) {
	idx := 0
	if s.ct == nil {
...
func (r *randGen) generateParticularCall(s *state, meta *Syscall) (calls []*Call, ok bool) {
	// Do not generate calls for new resource
	if _, ok := meta.Ret.(*ResourceType); ok {
		return calls, false
	}
	...
	c.Args, calls, ok = r.generateArgs(s, meta.Args)
	if !ok {
		return calls, false
	}
...
func (target *Target) GenerateAllSyzProg(rs rand.Source) *Prog {
	...
	for _, meta := range target.Syscalls {
		...
		calls, _ := r.generateParticularCall(s, meta)
		for _, c := range calls {
			s.analyze(c)
...
func (r *randGen) generateArgs(s *state, types []Type) ([]Arg, []*Call, bool) {
	var calls []*Call
	args := make([]Arg, len(types))

	// Generate all args. Size args have the default value 0 for now.
	for i, typ := range types {
		arg, calls1, ok := r.generateArg(s, typ)
		if !ok {
			return args, calls, false
		}
		if arg == nil {
	...
	return args, calls, true
}

func (r *randGen) generateArg(s *state, typ Type) (arg Arg, calls []*Call, ok bool) {
	return r.generateArgImpl(s, typ, false)
}

func (r *randGen) generateArgImpl(s *state, typ Type, ignoreSpecial bool) (arg Arg, calls []*Call, ok bool) {
	if typ.Dir() == DirOut {
		...
		case *IntType, *FlagsType, *ConstType, *ProcType,
			*VmaType, *ResourceType:
			return typ.DefaultArg(), nil, true
		}
	}
	...
	if typ.Optional() && r.oneOf(5) {
		if res, ok := typ.(*ResourceType); ok {
			...
			return MakeResultArg(typ, nil, v), nil, true
		}
		return typ.DefaultArg(), nil, true
	}
	...
	if pt, ok := typ.(*PtrType); ok && typ.Optional() {
		...
		case *StructType, *ArrayType, *UnionType:
			...
			if r.recDepth[name] >= 3 {
				return MakeSpecialPointerArg(typ, 0), nil, true
			}
		}
	}
	...
	if !ignoreSpecial && typ.Dir() != DirOut {
		...
		case *StructType, *UnionType:
			if gen := r.target.SpecialTypes[typ.Name()]; gen != nil {
				arg, calls = gen(&Gen{r, s}, typ, nil)
				return arg, calls, true
			}
		}
	}
...
func (a *ResourceType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	// switch {
...
func (a *BufferType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	switch a.Kind {
	case BufferBlobRand, BufferBlobRange:
		...
		if a.Dir() == DirOut {
			return MakeOutDataArg(a, sz), nil, true
		}
		data := make([]byte, sz)
		for i := range data {
			data[i] = byte(r.Intn(256))
		}
		return MakeDataArg(a, data), nil, true
	case BufferString:
		data := r.randString(s, a)
		if a.Dir() == DirOut {
			return MakeOutDataArg(a, uint64(len(data))), nil, true
		}
		return MakeDataArg(a, data), nil, true
	case BufferFilename:
		if a.Dir() == DirOut {
		...
		return MakeDataArg(a, []byte(r.filename(s, a))), nil, true
	case BufferText:
		if a.Dir() == DirOut {
			return MakeOutDataArg(a, uint64(r.Intn(100))), nil, true
		}
		return MakeDataArg(a, r.generateText(a.Text)), nil, true
	default:
		panic("unknown buffer kind")
	}
}

func (a *VmaType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	npages := r.randPageCount()
	if a.RangeBegin != 0 || a.RangeEnd != 0 {
		npages = a.RangeBegin + uint64(r.Intn(int(a.RangeEnd-a.RangeBegin+1)))
	}
	return r.allocVMA(s, a, npages), nil, true
}

func (a *FlagsType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	return MakeConstArg(a, r.flags(a.Vals)), nil, true
}

func (a *ConstType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	return MakeConstArg(a, a.Val), nil, true
}

func (a *IntType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	v := r.randInt()
	switch a.Kind {
	case IntFileoff:
		switch {
	...
	return MakeConstArg(a, v), nil, true
}

func (a *ProcType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	return MakeConstArg(a, r.rand(int(a.ValuesPerProc))), nil, true
}

func (a *ArrayType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	var count uint64
	switch a.Kind {
	case ArrayRandLen:
	...
	for i := uint64(0); i < count; i++ {
		arg1, calls1, _ok := r.generateArg(s, a.Type)
		if !_ok {
			return nil, calls, false
		}
		inner = append(inner, arg1)
		calls = append(calls, calls1...)
	}
	return MakeGroupArg(a, inner), calls, true
}

func (a *StructType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	args, calls, _ok := r.generateArgs(s, a.Fields)
	if !_ok {
		return nil, calls, _ok
	}
	group := MakeGroupArg(a, args)
	return group, calls, _ok
}

func (a *UnionType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	optType := a.Fields[r.Intn(len(a.Fields))]
	opt, calls, _ok := r.generateArg(s, optType)
	if !_ok {
		return nil, calls, _ok
	}
	return MakeUnionArg(a, opt), calls, _ok
}

func (a *PtrType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	if r.oneOf(1000) {
		index := r.rand(len(r.target.SpecialPointers))
		return MakeSpecialPointerArg(a, index), nil, true
	}
	inner, calls, _ok := r.generateArg(s, a.Type)
	if !_ok {
		return nil, calls, _ok
	}
	arg = r.allocAddr(s, a, inner.Size(), inner)
	return arg, calls, _ok
}

func (a *LenType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
	// Updated later in assignSizesCall.
	return MakeConstArg(a, 0), nil, true
}

func (a *CsumType) generate(r *randGen, s *state) (arg Arg, calls []*Call, ok bool) {
