...
func (wq *WorkQueue) enqueue(item interface{}) {
	...
	case *CapTriage:
		wq.captriage = append(wq.captriage, item)
	case *WorkTriage:
...
func (wq *WorkQueue) dequeue() (item interface{}) {
	...
	if len(wq.captriage) != 0 {
		last := len(wq.captriage) - 1
		item = wq.captriage[last]
		wq.captriage = wq.captriage[:last]
