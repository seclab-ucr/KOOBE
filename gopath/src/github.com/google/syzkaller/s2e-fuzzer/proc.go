...
func (proc *Proc) rndN(min, max int, power float64) int {
	randomfloat := proc.rnd.Float64()
	float_min, float_max := float64(min), float64(max)
	res := int(math.Floor(float_min + (float_max-float_min)*math.Pow(randomfloat, power)))
	return res
}
...
func (proc *Proc) loop() {
	...
	for i := 0; ; i++ {
		...
		if len(corpus) == 0 { //|| i%generatePeriod == 0 {
			...
			// mutate new test cases with higher likelihood
			p := corpus[proc.rndN(0, len(corpus), 0.2)].Clone()
...
func (proc *Proc) triageCap(item *CapTriage) {
	// FIXME: is the size check necessary?
	if item.cap.Flags == 0 { // || item.cap.Size == 0 {
		return
	}

	data := item.p.Serialize()
	sig := hash.Hash(data)
	log.Logf(2, "added new input for %v to corpus:\n%s", item.cap.Flags, data)
	proc.fuzzer.sendS2EInputToManager(rpctype.S2EInput{
		Prog:  data,
		Flags: item.cap.Flags,
	})

	proc.fuzzer.addS2EInputToCorpus(item.p, item.cap.Flags, sig)
}
...
func (proc *Proc) execute(execOpts *ipc.ExecOpts, p *prog.Prog, flags ProgTypes, stat Stat) *ipc.ProgInfo {
	...
	// No overflow
	if info.Cap.Size == 0 && info.Cap.Flags == 0 {
		return info
	}
	...
	proc.enqueueCapTriage(p, info.Cap)
	return info
}

func (proc *Proc) enqueueCapTriage(p *prog.Prog, cap ipc.Capability) {
	if cap.Flags == 0 {
		return
	}
	proc.fuzzer.workQueue.enqueue(&CapTriage{
		p:   p.Clone(),
		cap: cap,
	})
}
...
func (proc *Proc) executeRaw(opts *ipc.ExecOpts, p *prog.Prog, stat Stat) *ipc.ProgInfo {
	...
	for try := 0; ; try++ {
		...
		proc.logCapability(info)
		log.Logf(2, "result hanged=%v: %s", hanged, output)
		return info
	}
}

func (proc *Proc) logCapability(info *ipc.ProgInfo) {
	...
	case OutputStdout:
		...
		fmt.Printf("%02v:%02v:%02v S2E: %v:%v\n",
			now.Hour(), now.Minute(), now.Second(), info.Cap.Flags, info.Cap.Size)
		proc.fuzzer.logMu.Unlock()
	case OutputDmesg:
		fd, err := syscall.Open("/dev/kmsg", syscall.O_WRONLY, 0)
		if err == nil {
			buf := new(bytes.Buffer)
			fmt.Fprintf(buf, "S2E: %v:%v\n", info.Cap.Flags, info.Cap.Size)
