...
#ifdef S2E
	uint8_t cmd[1] = {'s'};
	s2e_invoke_plugin("Instrumentor", cmd, 1);
#endif
...
#ifdef S2E
	struct INTRUMENT_COMMAND query;
	query.Command = INSTRUMENT_QUERY;
	s2e_invoke_plugin("Instrumentor", &query, sizeof(query));
	write_output(0xdeadbeef);
	write_output(query.query.type);
	write_output(query.query.size);
#endif
}
