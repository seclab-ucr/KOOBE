...
type RPCManagerView interface {
	...
	newS2EInput(inp rpctype.S2EInput, origin rpctype.RPCInput) bool
...
func (serv *RPCServer) Connect(a *rpctype.ConnectArgs, r *rpctype.ConnectRes) error {
	...
	log.Logf(0, "Connect: %d", len(corpus))
...
func (serv *RPCServer) NewS2EInput(a *rpctype.NewS2EInputArgs, r *int) error {
	if _, err := serv.target.Deserialize(a.S2EInput.Prog, prog.NonStrict); err != nil {
		// This should not happen, but we see such cases episodically, reason unknown.
		log.Logf(0, "failed to deserialize program from fuzzer: %v\n%s", err, a.S2EInput.Prog)
		return nil
	}
	// serv.mu.Lock()
	// defer serv.mu.Unlock()
	go func() {
		...
		res := serv.mgr.newS2EInput(a.S2EInput, inp)
		if res {
			// for _, f := range serv.fuzzers {
			// 	if f.name == a.Name {
			// 		continue
