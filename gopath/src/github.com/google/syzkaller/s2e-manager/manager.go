...
func (mgr *Manager) loadCorpus() {
	...
	for key, rec := range mgr.corpusDB.Records {
		...
		for _, c := range p.Calls {
			if !syscalls[c.Meta.ID] {
				log.Logf(0, "disabled syscall %s", c.Meta.Name)
				disabled = true
				break
			}
		}
		...
		// Also add it into corpus
		mgr.corpus[hash.String(rec.Val)] = rpctype.RPCInput{
			Call:   ".extra",
			Prog:   rec.Val,
	...
	// }
	// duplicate N for N instances
	vmCount := mgr.vmPool.Count()
	for i := 0; i < vmCount; i++ {
		mgr.candidates = append(mgr.candidates, mgr.candidates[0])
	}
...
func (mgr *Manager) runInstance(index int) (*Crash, error) {
	...
	outc, errc, err := inst.Run(30*time.Minute, mgr.vmStop, cmd)
	if err != nil {
...
func (mgr *Manager) newS2EInput(inp rpctype.S2EInput, origin rpctype.RPCInput) bool {
	res := true
	mgr.mu.Lock()
	sig := hash.String(inp.Prog)
	if _, ok := mgr.corpus[sig]; ok {
		...
		mgr.corpus[sig] = origin // add hash to avoid replication
	}
	mgr.mu.Unlock()
...
func (mgr *Manager) candidateCorpus(size int) []rpctype.RPCCandidate {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()
	var res []rpctype.RPCCandidate
	for key, rec := range mgr.corpusDB.Records {
		_, err := mgr.target.Deserialize(rec.Val, prog.NonStrict)
		if err != nil {
			mgr.corpusDB.Delete(key)
			continue
		}
		res = append(res, rpctype.RPCCandidate{
			Prog:      rec.Val,
			Minimized: true,
			Smashed:   true,
		})
